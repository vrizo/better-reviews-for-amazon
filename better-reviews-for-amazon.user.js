...
(() => {
  ...
  const STAR_FILTERS = Object.freeze({
    POSITIVE: 'positive',
    CRITICAL: 'critical',
  });
  ...
  async function waitForAnchor(timeoutMs = 15000) {
  ...
  function findAnchor() {
    return (
      document.querySelector('#averageCustomerReviews_feature_div #averageCustomerReviews') ||
  ...
  async function getReviewStats(asin) {
    ...
    const staleCache = readCache(cacheKey, { allowExpired: true });
  ...
  function renderStats(container, stats) {
    ...
    const vineTitle = `Calculated as ${formatCount(stats.allCount)} all reviews minus ${formatCount(stats.verifiedCount)} verified-purchase reviews.`;
    vineRow.appendChild(makeTextStat('Vine reviews', stats.vineCount, vineTitle));
  ...
  function renderError(container, error) {
    ...
    message.textContent = /signed-in session/i.test(error?.message || '')
  ...
  function makeInlineLinkedStat(label, value, href) {
    ...
    wrapper.append(
      makeLinkedLabel(label, href),
      document.createTextNode(': '),
      makeBoldText(formatCount(value))
    );
    return wrapper;
  }

  function makeTextStat(label, value, title) {
    const wrapper = document.createElement('span');
    if (title) {
      wrapper.title = title;
    }
